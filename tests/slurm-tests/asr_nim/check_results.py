...
def check_server_startup(workspace: str, server_timeout: int):
    ...
    soft_assert(
        server_logs_dir.exists(),
        f"Server logs directory not found: {server_logs_dir}"
    )
    ...
    soft_assert(
        len(log_files) > 0,
        f"No log files found in {server_logs_dir}"
    )
    ...
    soft_assert(
        server_started,
        "Server startup indicators not found in logs"
    )
...
def load_references(workspace: str):
    ...
    """
    # The original test file should be in the workspace
    test_file = Path(workspace) / "asr.test"
...
def check_generation_results(workspace: str):
    ...
    soft_assert(
        len(output_files) > 0,
        f"No output JSONL files found in {output_dir}"
    )
    ...
    expected_samples = 2
    soft_assert(
        len(all_outputs) == expected_samples,
        f"Expected {expected_samples} outputs, found {len(all_outputs)}"
    )
    ...
    soft_assert(
        len(empty_transcripts) == 0,
        f"Empty transcripts for samples: {', '.join(empty_transcripts)}"
    )
    ...
    if not empty_transcripts:
        ...
        if word_match_results:
            ...
            all_passed = all(r['all_words_present'] for r in word_match_results)
...
def main():
    ...
    parser.add_argument(
        ...
        default="full",
        help="Test mode to validate"
    )
    parser.add_argument(
        "--server_timeout",
        type=int,
        default=180,
        help="Maximum seconds allowed for server startup"
    )
