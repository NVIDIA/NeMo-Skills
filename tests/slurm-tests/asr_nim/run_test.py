...
def load_nim_config(config_file, config_key):
    ...
    if config_key not in nim_configurations:
        raise ValueError(
            f"Configuration key '{config_key}' not found. "
            f"Available keys: {list(nim_configurations.keys())}"
        )
...
def prepare_test_data(workspace, cluster):
    """Upload test data and audio files to cluster, fix paths in JSONL.
    ...
    Returns:
        str: Path to the prepared input file on the cluster
    """
    from nemo_skills.pipeline.utils.cluster import cluster_upload, get_tunnel
    from nemo_skills.pipeline.utils import get_cluster_config, create_remote_directory
    ...
    # Get tunnel and establish connection
    tunnel = get_tunnel(cluster_config)
    tunnel.run("echo 'Establishing connection...'", hide=True)
    
    # Upload test data JSONL
    test_data_path = Path(__file__).parent / "asr.test"
    input_file_remote = f"{workspace}/asr.test"
    cluster_upload(cluster_config, test_data_path, input_file_remote, verbose=True)
    
    # Upload audio files directory
    wavs_dir = Path(__file__).parent / "wavs"
    wavs_dir_remote = f"{workspace}/wavs"
    
    # Create wavs directory on remote
    create_remote_directory(wavs_dir_remote, cluster_config)
    
    # Re-establish tunnel connection before uploading multiple files
    tunnel = get_tunnel(cluster_config)
    tunnel.run("echo 'Re-establishing connection...'", hide=True)
    
    # Upload each wav file with fresh connection attempts
    for wav_file in sorted(wavs_dir.glob("*.wav")):
        remote_wav_path = f"{wavs_dir_remote}/{wav_file.name}"
        print(f"Uploading {wav_file.name} to {remote_wav_path}")
        # Use rsync via tunnel for more robust transfer
        tunnel.run(f"mkdir -p {wavs_dir_remote}", hide=True)
        tunnel.put(str(wav_file), remote_wav_path)
    
    # Fix paths in the JSONL file to use absolute paths and remove reference field
    fixed_input_file = f"{workspace}/asr_fixed.test"
    
    # Read the original file, fix paths, remove reference, and write back
    ...
    for line in lines:
        if line.strip():
            ...
            if not audio_path.startswith("/"):
                # Make it absolute
                data["audio_path"] = f"{wavs_dir_remote}/{Path(audio_path).name}"
            # Remove reference field (will be used by checker separately)
            if "reference" in data:
                del data["reference"]
            fixed_lines.append(json.dumps(data) + "\n")
    
    # Create a temporary local file with fixed paths
    temp_fixed_file = Path(__file__).parent / "asr_fixed_temp.test"
    with open(temp_fixed_file, 'w') as f:
        f.writelines(fixed_lines)
    
    # Upload the fixed file
    cluster_upload(cluster_config, temp_fixed_file, fixed_input_file, verbose=True)
    
    # Clean up temporary file
    temp_fixed_file.unlink()
    
    return fixed_input_file
...
def run_generation_only(workspace, cluster, expname_prefix, server_host, server_port, server_node, nim_config):
    ...
    # Prepare test data (upload and fix paths)
    input_file = prepare_test_data(workspace, cluster)
    output_dir = f"{workspace}/asr_outputs"
    
    # Set up partition and GPU requirements based on whether we need to run on specific node
    if server_node:
        # When running on same node as server, use interactive partition with GPU
        partition = "interactive"
        num_gpus = 1
    else:
        # When running on any node, use CPU partition
        partition = "cpu"
        num_gpus = 0
...
def run_full_pipeline(workspace, cluster, expname_prefix, nim_config):
    ...
    generate(
        ...
        mount_paths=mount_paths,
        check_mounted_paths=True,
        exclusive=True,  # Use exclusive mode to get random ports
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Run ASR NIM slurm test in different modes"
    )
    ...
    parser.add_argument(
        "--server_host",
        default="127.0.0.1",
        help="Server host for generation-only mode"
    )
    parser.add_argument(
        "--server_port",
        default="8000",
        help="Server HTTP port for generation-only mode (gRPC will be port+1)"
    )
    parser.add_argument(
        "--server_node",
        default=None,
    ...
    parser.add_argument(
        "--skip_check",
        action="store_true",
        help="Skip the results check job"
    )
    parser.add_argument(
        "--config_file",
        default="recipes/asr_tts/nim_configurations.py",
        help="Path to NIM configurations file"
    )
    parser.add_argument(
        "--config_key",
