...
def check_generation_results(workspace: str):
    ...
    soft_assert(
        len(missing_audio) == 0,
        f"Missing audio files for samples: {', '.join(missing_audio)}"
    )
    
    soft_assert(
        len(empty_audio) == 0,
        f"Empty audio files for samples: {', '.join(empty_audio)}"
    )
    ...
    if not missing_audio and not empty_audio:
        ...
        total_size = sum(
            (Path(output.get('audio_path') or output.get('result')) 
             ...
             else audio_dir / Path(output.get('audio_path') or output.get('result', '')).name).stat().st_size
