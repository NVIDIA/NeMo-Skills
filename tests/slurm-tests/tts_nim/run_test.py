...
def run_generation_only(workspace, cluster, expname_prefix, server_host, server_port, server_node, nim_config):
    ...
    test_data_path = Path(__file__).parent / "tts.test"
    input_file = f"{workspace}/tts.test"
    output_dir = f"{workspace}/tts_outputs"
    
    # Setup workspace and get mount paths
    cluster_config, mount_paths = setup_workspace_and_mounts(workspace, cluster)
    
    # Upload test data from dev desktop to cluster
    # Need to ensure tunnel connection is established first
    tunnel = get_tunnel(cluster_config)
    tunnel.run("echo 'Establishing connection...'", hide=True)
    cluster_upload(cluster_config, test_data_path, input_file, verbose=True)
...
def main():
    parser = argparse.ArgumentParser(
        description="Run TTS NIM slurm test in different modes"
    )
