...
class NemoRLTask:
    ...
    def get_preamble_cmd(self):
        ...
        # cmd = "uv run --active which python "
        # cmd = "uv run --active python -c \"from nemo_rl.algorithms.grpo import MasterConfig, grpo_train, setup; print('Successful import.')\""
        return cmd

    def get_script_module(self):
        return "/opt/nemo-rl/examples/run_grpo_math.py"

    def get_job_cmd(self):
        ray_job_cmd = self.get_ray_launch_cmd()
        ray_job_cmd = (
            f"echo 'Starting training' && "
            f"{ray_job_cmd} uv run --active python {self.get_script_module()} "
...
def nemo_rl(
    ...
    # Check if custom NemoRLTask is provided via ctx.obj['ppo_task'], use that if available
    if hasattr(ctx, 'obj') and ctx.obj is not None and isinstance(ctx.obj, dict) and 'ppo_task' in ctx.obj:
        ppo_task = ctx.obj['ppo_task']  # type: type(NemoRLTask)
        assert isinstance(ppo_task, NemoRLTask), "`ppo_task` must be a subclass of NemoRLTask"
    else:
        ppo_task = None
