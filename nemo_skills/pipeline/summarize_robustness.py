...
def calculate_metric_range(input_files):
    ...
    file_metrics = {'symbolic_correct': [], 'judge_correct': []}

    for input_file in input_files:
        metrics = calculate_single_metric(input_file)

        for k in ['symbolic_correct', 'judge_correct']:
            if k in metrics['pass@1']:
                file_metrics[k].append(metrics['pass@1'][k])

    # Compute the range for each metric
    metric_range = dict()
    for metric_type, metric_list in file_metrics.items():
        if metric_list:
            metric_range[metric_type] = {
                "min": round(np.min(metric_list), 2),
                "max": round(np.max(metric_list), 2),
                "mean": round(np.mean(metric_list), 2)
            }
    return metric_range
...
def summarize_robustness(
    ...
    for benchmark_path in sorted(benchmarks_paths):  # sorting to ensure consistent order
        ...
        rob_metrics_to_print[benchmark]['symbolic_correct']['cons_rate'] = consistency_rate
    ...
    try:
        results['robustness'] = rob_metrics_to_print
