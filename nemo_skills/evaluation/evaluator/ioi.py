...
from nemo_codegen import utils
...
""")
        ...
        setup_result = worker_sandbox.execute_code(code=setup_script, language='bash')
        ...
        compile_result = worker_sandbox.execute_code(code=compile_command, language='bash')
        ...
        run_result = worker_sandbox.execute_code(code=run_command, language='bash')

        result.update({
            "run_success": not run_result.get('stderr'),
        ...
        worker_sandbox.execute_code(code=f"rm -rf {unique_dir}", language='bash')


def extract_final_cpp_block(text):
    # This pattern matches a code block starting with ```cpp or ```Cpp followed by a newline,
    # capturing everything up to the next ```
    pattern = r"```(?:cpp|Cpp)\s*\n(.*?)```"
    # re.DOTALL makes the dot match newlines so that the block content is fully captured.
    matches = re.findall(pattern, text, re.DOTALL)
    # Return the last match if any, otherwise an empty string.
...
def eval_ioi(cfg):
    ...
    for jsonl_file in unroll_files(cfg.input_files):
        ...
        for x, entry in enumerate(samples):
            ...
            get_output = re.findall(r'```(.*?)```', completion, re.DOTALL)
            completion = get_output[0] if len(get_output) > 0 else completion
            completion = re.sub(r'^(?:\n)?[Cc]pp\s*', '', completion)

            print("before", completion)
            completion = add_includes(completion, entry['ioi_id'])

            print("after processing", completion)

            test_case_results = {}
            problem_name = entry['name']
            if problem_name not in metadata:
                print(f"Problem {problem_name} not found in metadata")
                continue
            ...
            for subtask, subtask_data in problem_metadata.items():
                ...
                for i in range(0, len(test_items), batch_size):
                    ...
                    tasks = []
                    for test_name, test_data in batch:
                        task_args = {
                    ...
                    # Record each test result along with its test name and break if any test fails.
        ...
        output_path = (jsonl_file[:-6] + '_eval_results.json')
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
        utils.jdump(outputs, output_path, mode='wt')
