...
class EvaluateResultsConfig:
    ...
    eval_config: BaseEvaluatorConfig = field(default_factory=BaseEvaluatorConfig)
...
def evaluate_results(cfg: EvaluateResultsConfig):
    ...
    if cfg.eval_config.remove_thinking:
        LOG.info(
            f"Removing the thinking part from the {cfg.eval_config.generation_key} key "
            f"(using {cfg.eval_config.thinking_begin} and {cfg.eval_config.thinking_end} tokens). "
        ...
        for jsonl_file in unroll_files(cfg.input_files):
            with open(jsonl_file, encoding="utf-8") as f:
                ...
                for sample in samples:
                    if cfg.eval_config.generation_key not in sample:
                        raise ValueError(
                            f"Key {cfg.eval_config.generation_key} not found in a sample, but remove_thinking=True is specified. "
                            "Use eval_config.generation_key parameter to specify the key containing the generations."
                        )
            with open(jsonl_file, "wt", encoding="utf-8") as f:
                for sample in samples:
                    remove_thinking(
                        sample,
                        cfg.eval_config.generation_key,
                        cfg.eval_config.thinking_begin,
                        cfg.eval_config.thinking_end,
                    )
