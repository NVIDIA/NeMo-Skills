...
def load_dataset_stream(dataset_path, num_rows=1000):
    """Load dataset from HuggingFace and stream only a specified number of rows"""
    dataset_iter = load_dataset(dataset_path, split="train", streaming=True)
    dataset_list = list(dataset_iter.take(num_rows))
...
def load_corpus(corpus_path: str):
    if corpus_path.endswith(".jsonl"):
        corpus = datasets.load_dataset(
            "json", data_files=corpus_path, split="train", num_proc=4, download_mode="force_redownload"
        )
    elif corpus_path.startswith("inclusionAI"):
        corpus = load_dataset_stream(corpus_path, 1000000)
    else:
        raise ValueError(f"Unsupported corpus path: {corpus_path}")
    return corpus
