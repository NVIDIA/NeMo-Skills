    ...
    with open(output_file, "wt", encoding="utf-8") as fout:
        for idx, entry in tqdm(enumerate(data), desc=f"Writing {output_file.name}"):
            ...
            for data_source_filename in data_source_filenames:
                try:
                    with open(f"{txt_file_folder}/{document_set_id}/{data_source_filename}", "rt", encoding="utf-8") as fin:
                        document = fin.read()
                except FileNotFoundError:
                    print(f"File {txt_file_folder}/{document_set_id}/{data_source_filename}")
                    document = ""
                    continue
                question += document
...
def get_aalcr_data(txt_file_folder, max_context_window, setup, tokenizer_name):
    ...
    if txt_file_folder == "lcr-document-sets":
        txt_file_folder =  Path(__file__).absolute().parent / "lcr-document-sets"
