...
def get_args():
    ...
    parser.add_argument(
        ...
        default=None,
        required=True,
        help="Path to Huggingface Qwen3 checkpoints",
    )
...
def convert(args):
    ...
    model = Qwen3ForCausalLM.from_pretrained(args.in_path)
    ...
    for l in range(int(num_layers)):
        ...
        old_tensor_shape = model.t()[f'model.layers.{l}.self_attn.q_proj.weight'].size()
        ...
        # Qwen3 has additional RMSNorm for QK
        # TODO: verify
        # guessed from: https://github.com/NVIDIA/NeMo/blob/main/nemo/collections/llm/gpt/model/deepseek.py#L281
        q_proj_ln = model.state_dict()[f'model.layers.{l}.self_attn.q_norm.weight']
        k_proj_ln = model.state_dict()[f'model.layers.{l}.self_attn.k_norm.weight']
        if mcore_gpt:
            q_proj_ln_base_name = f'model.decoder.layers.{l}.self_attention.q_layernorm.weight'
            k_proj_ln_base_name = f'model.decoder.layers.{l}.self_attention.k_layernorm.weight'
        else:
            q_proj_ln_base_name = f'model.language_model.encoder.layers.{l}.q_layernorm.weight'
            k_proj_ln_base_name = f'model.language_model.encoder.layers.{l}.k_layernorm.weight'
        checkpoint['state_dict'][q_proj_ln_base_name] = param_to_weights(q_proj_ln)
        checkpoint['state_dict'][k_proj_ln_base_name] = param_to_weights(k_proj_ln)

        ###################### No bias for Qwen3 models
