...
docker run --network=host -e DEPLOYMENT_MODE=multi-worker -e NUM_WORKERS=24 -e UWSGI_PROCESSES=1 -e UWSGI_CHEAPER=1 -e KERNEL_READY_TIMEOUT=90 -e KERNEL_STARTUP_RETRIES=5 --rm --name=local-sandbox ${SANDBOX_NAME}
