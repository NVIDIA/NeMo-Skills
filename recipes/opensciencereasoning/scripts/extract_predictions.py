...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract predicted_answer, and optionally fill majority voting."
    )
        ...
        with open(destination, "w") as fout:
            for sample in samples:
                if args.majority_voting or "expected_answer" not in sample:
                    ...
                    sample["majority_voting_agreement_at_n"] = round(majority_votes / total_votes, 6) if total_votes else 0.0
