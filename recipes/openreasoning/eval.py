...
def eval_aai(model_size):
    eval(
        ctx=wrap_arguments(f"++inference.tokens_to_generate={eval_tokens} ++remove_thinking=True "),
        cluster=cluster,
...
def eval_math(model_size):
    ...
    eval(
        ctx=wrap_arguments(
            f"++inference.tokens_to_generate={eval_tokens} ++inference.temperature=0.6  ++remove_thinking=True "
        ),
        cluster=cluster,
...
def eval_science(model_size):
    eval(
        ctx=wrap_arguments(
            f"++inference.tokens_to_generate={eval_tokens} ++inference.temperature=0.6 ++prompt_config=eval/aai/mcq-4choices-boxed ++remove_thinking=True "
        ),
        cluster=cluster,
    ...
    eval(
        ctx=wrap_arguments(
            f"++inference.tokens_to_generate={eval_tokens} ++inference.temperature=0.6 ++prompt_config=eval/aai/mcq-10choices-boxed ++remove_thinking=True "
        ),
        cluster=cluster,
