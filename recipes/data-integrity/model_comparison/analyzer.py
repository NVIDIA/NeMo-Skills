...
class OrganizedModelAnalyzer:
    ...
    def initialize_models(self):
        ...
        try:
            ...
            logger.info("⚠️  Warning: Could not load SentenceTransformer model")
            
        try:
            self.nlp = spacy.load('en_core_web_sm')
            logger.info("✅ spaCy model loaded")
        except:
            logger.info("⚠️  Warning: Could not load spaCy model")
    ...
    def generate_final_report(self):
        ...
        logger.info(f"📊 Total files generated: {len(os.listdir(self.subdirs['visualizations'])) + len(os.listdir(self.subdirs['data_outputs']))}")
        logger.info(f"🔗 Open {os.path.join(self.results_dir, 'README.md')} for navigation guide")
        logger.info(f"🎮 All UMAP visualizations are interactive! Open the HTML files in your browser.")
        logger.info(f"🌟 5 interactive UMAP visualizations + dual similarity analysis (semantic + ROUGE-L) available")
