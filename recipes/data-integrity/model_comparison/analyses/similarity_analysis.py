...
def analyze_semantic_similarity(df, subdirs, sentence_model=None):
    ...
    # Summary statistics
    similarity_summary = similarity_df.groupby(['generator_1', 'generator_2']).agg({
    ...
    sns.heatmap(avg_semantic_matrix, 
    ...
    # ROUGE-L heatmap
    sns.heatmap(avg_rouge_matrix, 
               xticklabels=generators, yticklabels=generators,
               annot=True, cmap='RdYlBu_r', center=0.5,
               square=True, fmt='.3f', mask=mask,
               cbar_kws={'label': 'ROUGE-L F1'}, ax=axes[1])
    axes[1].set_title('Average ROUGE-L Similarity')
    
    plt.suptitle(f'Similarity Comparison', fontsize=16, y=1.02)
    plt.tight_layout()
    ...
    plt.suptitle(f'Similarity Score Distributions', fontsize=16, y=1.02)
    plt.tight_layout()
