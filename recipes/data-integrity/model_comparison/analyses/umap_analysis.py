...
def analyze_response_embeddings_umap(df, subdirs, sentence_model):
    ...
    plot_df['input_preview'] = plot_df['input'].apply(
    ...
    fig = px.scatter(plot_df, 
                    ...
                    title=f'Interactive Response Embedding Landscape',
                    labels={
    ...
    html_path = os.path.join(subdirs['visualizations'], 
                            f"{model_names}_response_embeddings_umap.html")
    fig.write_html(html_path)
    logger.info(f"💾 UMap Interactive plot saved: {html_path}")
    
    if write_image:
        for ext in ['png', 'svg', 'pdf']:
            img_path = os.path.join(subdirs['visualizations'], 
                                    f"{model_names}_response_embeddings_umap.{ext}")
            fig.write_image(img_path, scale=5 if ext == 'png' else 1)
            logger.info(f"💾 UMap Image saved: {img_path}")
    
    # Save embedding coordinates
    save_data(subdirs, df, plot_df[['generator', 'umap_x', 'umap_y', 'word_count']], 
...
def analyze_input_response_mapping_umap(df, subdirs, sentence_model):
    ...
    fig = px.scatter(plot_df, 
                    ...
                    title=f'Interactive Input-Response Semantic Mapping',
                    labels={
                        'umap_x': 'UMAP Dimension 1',
                        'umap_y': 'UMAP Dimension 2',
                        'generator': 'Model',
    ...
    model_names = get_model_comparison_name(df)
    html_path = os.path.join(subdirs['visualizations'], 
                            f"{model_names}_input_response_mapping.html")
    fig.write_html(html_path)
    ...
    if write_image:
        for ext in ['png', 'svg', 'pdf']:
            img_path = os.path.join(subdirs['visualizations'], 
                                    f"{model_names}_input_response_mapping.{ext}")
            fig.write_image(img_path, scale=5 if ext == 'png' else 1)
            logger.info(f"💾 UMap Image saved: {img_path}")
...
def analyze_multimodal_space_umap(df, subdirs, sentence_model):
    ...
    fig = px.scatter(plot_df, 
                    ...
                    title=f'Interactive Multimodal Space (Semantic + Stylistic)',
                    labels={
                        'umap_x': 'UMAP Dimension 1',
                        'umap_y': 'UMAP Dimension 2',
    ...
    model_names = get_model_comparison_name(df)
    html_path = os.path.join(subdirs['visualizations'], 
                            f"{model_names}_multimodal_space.html")
    fig.write_html(html_path)
    logger.info(f"💾 Interactive plot saved: {html_path}")
    ...
    if write_image:
        for ext in ['png', 'svg', 'pdf']:
            img_path = os.path.join(subdirs['visualizations'], 
                                    f"{model_names}_multimodal_space.{ext}")
            fig.write_image(img_path, scale=5 if ext == 'png' else 1)
            logger.info(f"💾 UMap Image saved: {img_path}")
    
    # Create a second plot colored by model instead of readability
    # Fix negative readability scores for size parameter
    plot_df['readability_normalized'] = plot_df['readability'] - plot_df['readability'].min() + 1  # Shift to positive values
    ...
    fig2 = px.scatter(plot_df, 
                     ...
                     title=f'Interactive Multimodal Space by Model (Size = Readability)',
                     labels={
                         'umap_x': 'UMAP Dimension 1',
                         'umap_y': 'UMAP Dimension 2',
                         'model_display': 'Model',
    ...
    # Save second plot
    html_path2 = os.path.join(subdirs['visualizations'], 
                             f"{model_names}_multimodal_by_model.html")
    fig2.write_html(html_path2)
    logger.info(f"💾 Second interactive plot saved: {html_path2}")

    if write_image:
        for ext in ['png', 'svg', 'pdf']:
            img_path2 = os.path.join(subdirs['visualizations'], 
                                    f"{model_names}_multimodal_by_model.{ext}")
            fig2.write_image(img_path2, scale=5 if ext == 'png' else 1)
            logger.info(f"💾 UMap Image saved: {img_path2}")
    
    # Save multimodal data
    save_data(subdirs, df, plot_df[['generator', 'umap_x', 'umap_y', 'readability', 
                           'sentiment_polarity', 'sentiment_subjectivity', 'unique_words']], 
