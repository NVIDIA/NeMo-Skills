            ...
            if len(answer_clusters) < 2:
                ...
                if len(answer_clusters) == 1:
                    ...
                    single_answer_instance = {
                        ...
                        "is_correct": (is_correct_judgement(instance["judgement"]) if "judgement" in instance else instance["is_correct"]),
                    }
                else:
                    single_answer_instance = {
                        "problem": problem, 
...
def create_comparison_instance(clustered_instances, problem, max_samples=8, sampling_strategy="linear"):
    ...
    sampled_instances = sample_instances(clustered_instances, max_samples=max_samples, sampling_strategy=sampling_strategy)
...
def main(input_dir, output_dir=None, max_samples=8, sampling_strategy="linear", num_random_seeds=8, num_input_samples=8):    
    ...
    for random_seed in range(num_random_seeds):
        ...
        with open(os.path.join(output_dir, f"output-rs{random_seed}.jsonl"), "w") as f:
            for problem, clustered_instances in problem_to_clustered_instances.items():
                comparison_instance = create_comparison_instance(clustered_instances, problem, max_samples=max_samples, sampling_strategy=sampling_strategy)
...
if __name__ == "__main__":
    ...
    random.seed(42)
    main(args.input_dir, args.output_dir, args.max_samples, args.sampling_strategy, args.num_random_seeds, args.num_input_samples)
