...
def step_1_preprocess_data(
    ...
    preprocess_command = (
        f"python /nemo_run/code/recipes/omr1/scripts/preprocess_genrm.py --input_dir {input_dir} --output_dir {comparison_dir} --max_samples {max_samples} --sampling_strategy {sampling_strategy} --num_random_seeds {num_random_seeds}"
        + (f" --num_input_samples {num_input_samples}" if num_input_samples is not None else "")
    )
...
def step_2_score_solns(
    ...
    for random_seed in range(num_random_seeds):
        ...
        generate(
            ...
            server_type="trtllm",
            server_gpus=os.environ["NUM_GPUS"],
            server_nodes=1,
            partition=partition,
            dependent_jobs=0,
            expname=f"{exp_name}",
...
def step_3_postprocess_judgment(
    cluster, partition, last_exp_name, step_1_output_dir, step_2_output_dir, output_dir, benchmark
):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of solutions with different random seeds used for reranking.",
    )
